...
const InventoryTable = (props) => {
  const { items, onEdit, onDelete, onStockEdit } = props;
  if (items.length === 0) {
    return (
                  ...
                  <tr key={item.id} className="hover:shadow-sm transition-shadow even:bg-gray-50">
                    ...
                    <td className="px-6 py-5 align-top whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => onStockEdit(item)}
                          className="p-2 rounded-md border border-transparent text-blue-600 hover:bg-blue-50"
